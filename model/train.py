...
import os
import logging
from model.config_loaders import load_datamodule_from_config, load_litmodule_from_config, load_trainer_from_config
...
def check_args(args):
    ...
    assert all([os.path.exists(f) for f in args.datamodule_config]), "LightningDataModule configuration file(s) do(es) not exist."
